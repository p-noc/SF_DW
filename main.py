...
class QueryTester:
    ...
    def __init__(self):
        ...
        # Query 4 (Original)    #9
        self.queryArray.append("select q2.city,q2.call_type,q2.original_priority, avg(number_of_unit_dispatched) from (Select call_number, count(*) as number_of_unit_dispatched from dispatch911_original			group by call_number)as q1 inner join (Select distinct call_number, original_priority, call_type,city from dispatch911_original as fact)as q2 on q1.call_number = q2.call_number group by q2.original_priority,q2.call_type,q2.city order by 4")
        # Query 4 (Dimensions)  #10
        self.queryArray.append("select city,q2.call_type,q2.original_priority, avg(number_of_unit_dispatched) from (Select call_number, count(*) as number_of_unit_dispatched from dispatch911_dimensions			group by call_number)as q1 inner join (Select distinct call_number, original_priority, call_type, city from dispatch911_dimensions as fact inner join dim_call_type as emer	on fact.id_call_type = emer.id_call_type inner join	dim_geo_place as geop on fact.id_geo_place = geop.id_geo_place)as q2 on q1.call_number = q2.call_number group by q2.city,q2.original_priority,q2.call_type order by 4 ")
